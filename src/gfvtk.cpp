/* $Id: gfvtk.cpp 60 2008-05-23 07:00:03Z greg $ */
...
extern "C" {
  ...
  void position_cone(vtkConeSource *c, bond *bptr) {
    vector3 direction = sub_vector3(bptr->b->loc,bptr->a->loc);
    vector3 center = add_vector3(bptr->a->loc,div_by_scalar_vector3(direction,2));

    c->SetHeight(mag_vector3(direction));
    c->SetResolution(10);
  ...
  vtkvis_eo *new_vtkvis_eo(extended_object *eo,vtkRenderer *ren1,
    ...
    mass_element *meptr = eo->first_mass_element;
    int i=0;
  ...
  void update_vtkvis_eo(vtkvis_eo *eov) {
    ...
    vector3 torque_center = add_vector3(eo->center_of_mass,mul_by_scalar_vector3(eo->torque,eov->torque_scale/2));
    eov->torque_cone->SetRadius(eov->eo_vector_conebase);
    eov->torque_cone->SetHeight(mag_vector3(eo->torque)*eov->torque_scale);
    ...
    vector3 am_center = add_vector3(eo->center_of_mass,mul_by_scalar_vector3(eo->angular_momentum,eov->am_scale/2));
    eov->am_cone->SetRadius(eov->eo_vector_conebase);
    eov->am_cone->SetHeight(mag_vector3(eo->angular_momentum)*eov->am_scale);
    ...
    vector3 lv_center = add_vector3(eo->center_of_mass,mul_by_scalar_vector3(eo->linear_velocity,eov->lv_scale/2));
    eov->lv_cone->SetRadius(eov->eo_vector_conebase);
    eov->lv_cone->SetHeight(mag_vector3(eo->linear_velocity)*eov->lv_scale);
  ...
  vtkvis_univ *new_vtkvis_univ(universe *univ, vtkRenderer *ren1, vtkvis_scaling_factors sf) {
    ...
    extended_object *eoptr = univ->first_extended_object;
    //vtkvis_eo *eov;
    ...
    while(eoptr!=NULL) {
      ...
      compute_center_of_mass_eo(eoptr);
